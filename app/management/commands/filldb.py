...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if users_cnt:
            ...
            print('Users generated, time: {}s'.format(
                time.time() - start_users_time
                )
            )
        ...
        if questions_cnt:
            ...
            print('Questions generated, time: {}s'.format(
                time.time() - start_questions_time
                )
            )
        ...
        if likes_cnt:
            ...
            print('Likes generated, time: {}s'.format(
                time.time() - start_likes_time
                )
            )
    ...
    def fill_profiles(self, cnt):
        ...
        avatar_links = [f for f in listdir(avatars_path)
                        if isfile(join(avatars_path, f))]
    ...
    def fill_likes(self, cnt):
        ...
        self.bulk_create_in_batches(answer_likes_amount,
                                    answer_likes_generator, AnswerLike, True)
    ...
    def bulk_create_in_batches(self, cnt, generator, model_type,
        ...
        if cnt > BATCH_SIZE:
            ...
            for i in range(batch_amount):
                ...
                model_type.objects.bulk_create(
                    batch, BATCH_SIZE, ignore_conflicts=ignore_conflicts
                )
        else:
